...
class ChurchPlantThemeBundle {
	...
	public function __construct() {
		...
		$mp_theme_bundle_slug = $this->_theme_bundle_info['slug'];
		...
		*/
		...
		// Loop through each niche-misc-functions file and include it.
		foreach ( $this->_theme_bundle_info['niche_misc_functions'] as $niche_misc_functions_file ) {
			require $this->_theme_bundle_info_plugin_dir . 'includes/niche-misc-functions/' . $niche_misc_functions_file;
		}
	...
	*/
	...
	public function text_domain() {
		...
		$mofile_global = WP_LANG_DIR . '/church_plant_theme_bundle/' . $mofile;
		...
		if ( file_exists( $mofile_global ) ) {
			...
			load_plugin_textdomain( 'church_plant_theme_bundle', false, $lang_dir );
		}
	...
	*/
	...
	public function theme_bundle_dependencies() {
		...
		foreach ( $required_text_domain_strings as $text_domain => $plugin_checker_file ) {
			...
			if ( ! function_exists( $text_domain ) && ! class_exists( $text_domain ) ) {
				return false;
			}
		}
	...
	public function theme_bundle_install() {
		...
		$initial_version = get_option( 'mp_initial_version_' . $this->_theme_bundle_info['slug'] );

		// If we have not yet stored the initial version that was installed.
		if ( empty( $initial_version ) ) {
			update_option( 'mp_initial_version_' . $this->_theme_bundle_info['slug'], $this->_theme_bundle_info_version );
		}
		...
		// Attempt to activate the theme now. This covers an off-scenario where the theme is installed but not active.
		...
		// Hook to fire actions upon installation.
		do_action( 'mp_theme_bundle_activated', $mp_core_options['mp_stacks_theme_bundle_being_installed'] );
	}
	 ...
	 * @link     http://mintplugins.com/doc/
	 * @see      function_name()
	 * @param    array  $mp_core_options The options.
	 * @param    string $theme_bundle_slug The theme bundle slug.
	 ...
	 */
	public function set_up_default_stacks( $default_stacks_to_create ) {
	 ...
	 * @return   void
	 */
	public function theme_bundle_switch_theme_workaround() {
	 ...
	 * @return void
	 */
	public function theme_bundle_kill_edd_welcome() {
 ...
 * @access   public
 * @since    1.0.0
 * @return   ChurchPlantThemeBundle
 */
function church_plant_theme_bundle_function() {

	// Declare the global variable.
	global $church_plant_theme_bundle_global_var;

	// Check if that global variable is already instantiated.
	if ( $church_plant_theme_bundle_global_var instanceof ChurchPlantThemeBundle ) {
		// If it has, return it rather than running through the entire object set-up again.
		return $church_plant_theme_bundle_global_var;
	}
	...
	$church_plant_theme_bundle_global_var = new ChurchPlantThemeBundle();

	return $church_plant_theme_bundle_global_var;
}
church_plant_theme_bundle_function();
...
function church_plant_theme_bundle_dependencies() {
	return church_plant_theme_bundle_function()->theme_bundle_dependencies();
}
